    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[#1a1b1e] p-6 font-['Inter',_sans-serif]">
      <div className="w-full max-w-[420px] bg-[#24262b] rounded-[32px] p-10 shadow-2xl animate-in fade-in zoom-in-95 duration-500 border border-white/[0.02]">
        ...
        <div className="mt-12 flex items-center justify-center gap-2">
          {mode === 'login' && (
            <>
              <button type="button" onClick={() => setMode('forgot-password')} className="text-[#58a6ff] hover:underline text-[14px] font-bold border-none bg-transparent cursor-pointer">Забыли пароль?</button>
              <span className="text-zinc-700 font-bold">•</span>
              <button type="button" onClick={() => setMode('register')} className="text-[#58a6ff] hover:underline text-[14px] font-bold border-none bg-transparent cursor-pointer">Создать аккаунт</button>
            </>
          )}
          {mode !== 'login' && (
            <button type="button" onClick={() => setMode('login')} className="text-[#58a6ff] hover:underline text-[14px] font-bold border-none bg-transparent cursor-pointer">Вернуться ко входу</button>
          )}
        </div>
      </div>
    </div>

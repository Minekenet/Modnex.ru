...
const ProfilePage: React.FC = () => {
  ...
  // Состояние для выпадающих меню в списке проектов
  const [openMenuId, setOpenMenuId] = useState<string | null>(null);

  // Пагинация
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 2; // Малое число для демонстрации работы кнопок
  ...
  const saveCroppedImage = () => {
    if (cropType === 'avatar') {
      setProfileData({ ...profileData, avatarUrl: tempImageUrl });
    } else {
      setProfileData({ ...profileData, bannerUrl: tempImageUrl });
    }
    setIsCropOpen(false);
  };
        ...
        <div className="flex items-center gap-5">
          {pages.map(n => (
            <button 
              ...
              className={`text-[15px] font-black border-none bg-transparent cursor-pointer transition-all ${
  ...
  return (
    <div className="min-h-screen bg-[#1c1c1f] text-white font-['Inter',_sans-serif] pb-20 relative">
      ...
      <button 
        ...
        className="fixed bottom-4 left-4 z-[100] bg-white/5 text-[9px] px-3 py-1 rounded-full border-none cursor-pointer hover:bg-white/10 transition-all text-zinc-600 uppercase font-black"
      >
        {isOwnProfile ? 'Владелец' : 'Гость'}
      </button>
        ...
        <div className="bg-[#24262b] rounded-2xl overflow-hidden shadow-2xl mb-12 relative">
          ...
          >
            ...
            {profileData.bannerUrl ? (
              <img src={profileData.bannerUrl} className="w-full h-full object-cover opacity-60" alt="" />
            ) : (
              <div className="absolute inset-0 bg-gradient-to-br from-[#1c1c21] to-[#0a0a0c]"></div>
            )}
            ...
            {isOwnProfile && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="bg-white/10 backdrop-blur-md px-6 py-3 rounded-xl border border-white/10 text-[11px] font-black uppercase tracking-widest">Изменить баннер</span>
              </div>
            )}
          </div>
          
          {/* Info Block - Поправили отступы (pt-24) и размер ника (text-3xl) */}
          <div className="px-12 pb-12 pt-24 relative"> 
            <div className="flex flex-col md:flex-row items-end gap-10 -mt-36 mb-12">
              ...
              <div className="relative group shrink-0">
                ...
                <div 
                  ...
                  className={`w-48 h-48 rounded-2xl border-[10px] border-[#24262b] overflow-hidden bg-[#1a1b23] shadow-2xl transition-all ${isOwnProfile ? 'cursor-pointer hover:brightness-110' : ''}`}
                >
              ...
              <div className="flex-grow pb-4">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                  ...
                  <div className="flex gap-3 shrink-0">
                    {isOwnProfile ? (
                      <>
                        <button className="bg-white text-black hover:bg-zinc-200 px-8 py-4 rounded-xl text-[12px] font-black uppercase tracking-widest border-none cursor-pointer shadow-xl">Создать</button>
                        <button onClick={() => setIsSettingsOpen(true)} className="bg-zinc-800 hover:bg-zinc-700 text-white px-6 py-4 rounded-xl border-none cursor-pointer transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg></button>
                      </>
                    ) : (
                      <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-10 border-t border-white/5">
              {[
                { label: 'Скачиваний', val: '62.4K' },
                { label: 'Проекты', val: myMods.length },
                { label: 'Лайки', val: '1.2K' },
                { label: 'Репутация', val: '9.8' }
              ].map((stat, i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              ...
              />
              <select value={filterGame} onChange={(e) => { setFilterGame(e.target.value); setCurrentPage(1); }} className="bg-[#24262b] border-none px-6 py-4 rounded-xl text-sm font-bold text-zinc-400 outline-none appearance-none cursor-pointer min-w-[200px]">
                <option>Все игры</option><option>Minecraft</option><option>Cyberpunk 2077</option>
              </select>
            </div>
              ...
              {paginatedList.length > 0 ? paginatedList.map((item) => (
                <div key={item.id} className="group bg-[#24262b] p-5 rounded-xl flex items-center justify-between hover:bg-[#2a2c33] transition-all relative">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">{item.game} • {item.category}</span>
                      <h3 className="text-xl font-black text-zinc-100 uppercase tracking-tight m-0">{item.title}</h3>
                      <div className="flex gap-4 mt-3 text-[11px] font-bold text-zinc-600 uppercase tracking-widest">
                        <span>{item.downloads} downloads</span><span>{item.likes} likes</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="relative" ref={openMenuId === item.id ? menuRef : null}>
                    ...
                    {openMenuId === item.id && (
                      <div className="absolute right-0 top-full mt-2 w-56 bg-[#1a1b23] border border-white/5 rounded-xl shadow-2xl z-50 overflow-hidden py-2 animate-in fade-in zoom-in-95">
                        <button className="w-full text-left px-5 py-3 text-[12px] font-black uppercase tracking-widest hover:bg-white/5 transition-all bg-transparent border-none text-zinc-400 hover:text-white cursor-pointer flex items-center gap-3">
                        ...
                        <button className="w-full text-left px-5 py-3 text-[12px] font-black uppercase tracking-widest hover:bg-red-500/10 transition-all bg-transparent border-none text-zinc-600 hover:text-red-500 cursor-pointer flex items-center gap-3">
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="bg-[#24262b] rounded-xl p-8 border border-white/[0.03]">
              ...
              <div className="flex flex-col gap-3">
                <a href={profileData.boostyUrl} target="_blank" className="flex items-center gap-4 p-4 bg-orange-500/10 rounded-xl hover:bg-orange-500 transition-all group no-underline border border-orange-500/20">
                ...
                <a href={profileData.patreonUrl} target="_blank" className="flex items-center gap-4 p-4 bg-red-500/10 rounded-xl hover:bg-red-500 transition-all group no-underline border border-red-500/20">
      ...
      {isCropOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/95 backdrop-blur-xl" onClick={() => setIsCropOpen(false)}></div>
          <div className="relative bg-[#24262b] w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl border border-white/5 animate-in zoom-in-95">
            <div className="p-10">
              <h2 className="text-2xl font-black uppercase mb-8 tracking-tighter">Настройка {cropType === 'avatar' ? 'фото' : 'баннера'}</h2>
              <div className="bg-[#1a1b23] rounded-xl overflow-hidden mb-8 flex items-center justify-center relative min-h-[350px]">
                <div className={`overflow-hidden flex items-center justify-center ${cropType === 'avatar' ? 'w-64 h-64 rounded-2xl border-4 border-white/10' : 'w-full h-52 rounded-xl border border-white/5'}`}>
                  <img src={tempImageUrl} style={{ transform: `scale(${zoom})`, transition: 'transform 0.1s ease-out' }} className="max-w-none object-cover" alt="Preview" />
                </div>
              </div>
              <div className="space-y-4 mb-10">
                <div className="flex justify-between items-center"><label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Масштаб</label><span className="text-[10px] font-black text-white">{Math.round(zoom * 100)}%</span></div>
                <input type="range" min="1" max="3" step="0.01" value={zoom} onChange={(e) => setZoom(parseFloat(e.target.value))} className="w-full h-2 bg-[#1a1b23] rounded-full appearance-none cursor-pointer accent-white" />
              </div>
              <div className="flex gap-4">
                <button onClick={() => setIsCropOpen(false)} className="flex-grow bg-zinc-800 text-zinc-400 py-4 rounded-xl font-black uppercase tracking-widest text-[11px] border-none cursor-pointer">Отмена</button>
                <button onClick={saveCroppedImage} className="flex-grow bg-white text-black py-4 rounded-xl font-black uppercase tracking-widest text-[11px] border-none cursor-pointer">Сохранить</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/90 backdrop-blur-md" onClick={() => setIsSettingsOpen(false)}></div>
          <div className="relative bg-[#24262b] w-full max-w-xl rounded-xl overflow-hidden shadow-2xl p-10 border border-white/5">
            <div className="flex items-center justify-between mb-10">
              ...
              <button onClick={() => setIsSettingsOpen(false)} className="text-zinc-600 hover:text-white bg-transparent border-none cursor-pointer p-2"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>

...
const SupportChat: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    // Mock response since AI is removed
    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'model', text: 'AI поддержка сейчас отключена. Пожалуйста, свяжитесь с администрацией другим способом.' }]);
      setIsLoading(false);
    }, 1000);
  };
      ...
      <div ref={scrollRef} className="flex-grow p-6 overflow-y-auto no-scrollbar space-y-6 bg-[#0d0d0f]/50">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2`}>
            <div className={`max-w-[90%] px-5 py-3.5 rounded-2xl text-[14px] font-medium leading-relaxed shadow-sm ${m.role === 'user'
      ...
      <form onSubmit={handleSend} className="p-6 bg-[#24262b] border-t border-white/5 shrink-0">
        <div className="relative flex items-center">
          ...
          <button
            ...
            className={`absolute right-2 w-10 h-10 rounded-xl flex items-center justify-center transition-all border-none cursor-pointer ${input.trim() && !isLoading ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-600'
              }`}
          >
            {isLoading ? (
